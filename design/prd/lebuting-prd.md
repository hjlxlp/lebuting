...
| 版本 | v1.1 |
| 日期 | 2026-06-30 |
| 状态 | Agent / 吃什么 / 做什么已实现；前端 UI 已按 `design/html` 改版；记账为规划/原型阶段 |
...
| 吃什么菜 | `eat-dish` | `/api/eat-dish` | `eat_dish_*` | `pages/eat-dish/` | `utils/eat-dish.uts` | `#e85d5d` | ✅ 已实现 |
| 做什么菜 | `cook-dish` | `/api/cook-dish` | `cook_dish_*` | `pages/cook-dish/` | `utils/cook-dish.uts` | `#3d9b5c` | ✅ 已实现 |
| 记账 | `bill` | `/api/bill`（规划） | `bill_*`（规划） | `pages/bill/`（规划） | `utils/bill.uts`（规划） | `#e85d5d`（原型） | 📋 HTML 原型 |
...
└── [记账]             pages/bill/       （规划；首页入口暂显示「敬请期待」）

吃什么菜 / 做什么菜（结构对称，文案不同）
├── 转盘 wheel（默认进入，custom 顶栏 + 底栏 Tab）
├── 候选 foods
├── 排除 excluded（今天不吃 / 今天不做）
├── 记录 records
├── 统计 stats
├── 候选编辑 food-edit（系统导航栏）
├── 分类 types（系统导航栏）
└── 分类编辑 type-edit（系统导航栏）

记账（规划；HTML 原型见 `design/html/index.html` 中 bill-* 屏）
...
参考截图 / CSV / 桌面分析页（本地素材，路径可自定，如 `my/bill/`）
```
...
> HTML 原型：`design/html/`（`index.html`、`css/lebuting.css`）。App 落地时遵循 `.agents/skills/frontend-design/SKILL.md` 与 `uni-app-x` 样式约束。

| 项 | 规范 |
|----|------|
| 布局 | 浅灰底 `#f5f5f7` + 白卡片圆角 + 轻阴影；业务模块 5 Tab 底栏 |
| 层级 | 标题 / 主数据 / 辅助说明 / 操作按钮 |
| 模块色 | 各模块主色见 §3.3（以 HTML 原型为准），禁止随意混用 |
| 首页 | 模块卡片（图标圆底 + 标题 + 描述 + 箭头）；记账卡片标「敬请期待」 |
| 顶栏 | Tab 主页面 `navigationStyle: custom` + `components/topbar`；编辑/分类页用系统导航栏 |
| 共享组件 | `components/tab-bar`、`list-row`、`mod-card`、`wheel`、`form-card` 等；模块差异见 `utils/module-config.uts` |
| 删除确认 | 使用 `uni.showModal`，不单独建 modal 页面 |
...
| 路径 | 标题 | 说明 |
|------|------|------|
| `pages/chat/chat` | Agent | custom 顶栏 + 消息列表 + 输入栏 + 连接状态 |
...
| 路径 | 标题 | Tab |
|------|------|-----|
| `pages/eat-dish/wheel` | 吃什么 | 转盘 |
| `pages/eat-dish/foods` | 吃饭候选 | 候选 |
| `pages/eat-dish/excluded` | 今天不吃 | 不吃 |
| `pages/eat-dish/records` | 吃过记录 | 记录 |
| `pages/eat-dish/stats` | 吃饭统计 | 统计 |
| `pages/eat-dish/food-edit` | 新增/编辑候选 | 系统导航栏 |
| `pages/eat-dish/types` | 吃饭分类 | 系统导航栏 |
| `pages/eat-dish/type-edit` | 新增/编辑分类 | 系统导航栏 |

底栏组件：`components/tab-bar/tab-bar.uvue`（`module="eat-dish"`）
...
| `pages/cook-dish/type-edit` | 新增/编辑分类 | 系统导航栏 |

底栏：`components/tab-bar/tab-bar.uvue`（`module="cook-dish"`）
...
> **当前阶段**：App 未实现；HTML 交互原型已在 `design/html/`（含 bill-home / bill-add / bill-charts / bill-categories）；后端 API 暂缓，MVP 范围待产品确认。

### 9.1 概述

个人记账：记一笔、看首页概览、图表分析、分类管理。参考 App 截图、CSV 样例、桌面分析页等为**本地参考素材**（常见路径 `my/bill/`，非仓库必需）。
...
来源：参考 CSV（如 `20260111160124.csv`，本地 `my/bill/` 等）
...
`bill.html`（桌面宽屏分析工具，Chart.js）为**可选本地参考**，能力包括：
...
| 首页入口 | `pages/index/index.uvue` 四模块卡片；记账点击 toast「敬请期待」 |
| UI 来源 | `design/html/` 原型 → `pages/` + `components/` 落地（除记账 App 页） |
...
|------|------|------|
| 1 | 整理完善本 PRD | ✅ |
| 2 | 统一 UI HTML 原型 `design/html/` | ✅ |
| 3 | 除记账外的前端 UI 改版（对齐原型） | ✅ |
| 4 | 除记账外的后端 / API 联调（业务 API 已有） | ✅ 维护 |
| 5 | 记账：PRD + HTML 原型；App / 后端实现不急 | 进行中 |
...
| 共享 UI | `components/`（`topbar`、`tab-bar`、`list-row`、`wheel` 等） |
| 设计令牌 | `styles/tokens.scss`、`styles/layout.scss`、`uni.scss` |
| 模块配置 | `utils/module-config.uts` |
| HTML 原型 | `design/html/index.html`、`design/html/css/lebuting.css` |
| 后端入口 | `backend/main.py` |
| 数据库 | `backend/data/lebuting.db` |
| 记账参考素材 | 本地可选（截图 / CSV / `bill.html`） |
...
| v1.1 | 2026-06-30 | 同步 UI 改版：主色、组件路径、`design/html` 原型、路线图状态 |
